...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (child <= last) {
            if (child + 1 <= last
                    && order.compare(array[child], array[child + 1]) > 0) {
                child = child + 1;
            }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        if (array.length > 1) {
            ...
            if (left <= last && !isHeap(array, left, last, order)) {
                heapify(array, left, order);
            }

            if (left + 1 <= last && !isHeap(array, left + 1, last, order)) {
                heapify(array, left + 1, order);
            }
